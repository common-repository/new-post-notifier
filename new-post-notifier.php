...
function em_setting_api_mail($subject, $message) {
	...
	if (is_array(get_option('em_notification_options'))) {
		...
		foreach($data as $datum) {
			if(!empty($outputs['em_post_notifier'][$datum->ID])) {
				return em_mail($datum->user_email, $subject, $message);
			}
		}
	}
}
...
function em_editPost($id) {
	$id = $id;
	$result = em_get_data_post($id);
	$rolePost = ucfirst(em_get_role_post($result->post_author));
	$currentLogin = em_get_current_user_role();
...
function em_trashed($id) {
	...
	$rolePost = ucfirst(em_get_role_post($result->post_author));
	$currentLogin = em_get_current_user_role();
	if (ucfirst($result->user_email) != 'Administrator') {
		if (ucfirst($currentLogin) != $rolePost) {
...
function em_notification_initialize_options() {
	...
	add_settings_field('em_post_notifier', 'Administrator:', 'em_notification_setting', __FILE__, 'em_main_section');
...
?>
	...
	<?php
		...
		foreach ($authors as $author) {
			$id = $author->ID;
				$html = "<li> <input type='checkbox' name='em_notification_options[em_post_notifier][$id]' value='1'";
				$html .= ((isset($options['em_post_notifier'][$id])) ? checked( 1, $options['em_post_notifier'][$id], false ) : '')  ."/>";
